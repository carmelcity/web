...
export default ({ data }: { data: DataItem }) => {
  return (
    <div onClick={data.onSelect} className={`cursor-${data.simple ? 'auto' : 'pointer'} flex flex-col w-full`}>
      <div className="flex flex-row items-center w-full bg-black/50 p-2">
        <div className="flex items-center w-full px-4 py-4 border-b border-cyan/10">
          <div className="flex flex-col w-full">
            <div className="flex text-xl pt-2 break-words text-pretty [overflow-wrap:anywhere]">{data.title}</div>
            <div className="flex flex-row w-full text-grey text-sm flex-wrap mt-2 mb-2">
              <span>
                {data.type === 'team' || data.type === 'community'
                  ? 'founded by'
                  : data.type === 'user'
                  ? 'since'
                  : 'by'}
              ...
              {data.target && [
                ...
                <Link
                  key="sp2"
                  className={`${font} text-cyan font-normal pl-1 z-50 pointer-cursor`}
                  href={`/${data.target}`}>
                  {data.target}
                </Link>,
              ]}
          ...
          {data.funds && (
            <div className="flex flex-col items-center align-center items-center">
              <Link
                ...
                key={'xlink'}
                className={`btn border w-20 lg:w-24 text-white border-primary/50 ${
                  data.purchase ? 'bg-primary/50' : ''
                }`}>
