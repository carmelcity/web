...
import { ProfileHeaderNotEditablePlaceholder, ProfileHeaderPlaceholder } from '../placeholders/ProfileHeader';
import EditIcon from '~/images/dashboard/EditIcon.webp';
import PhotoIcon from '~/images/dashboard/PhotoIcon.webp';
import { useEffect, useState } from 'react';
import * as nft from '~/images/NFT.webp';
import { simulatePostRequest } from './hooks';
import { showErrorToast, showSuccessToast } from '../toasts';
import PROFILE_PLACEHOLDER from '~/images/profile_placeholder.webp';
import BANNER_PLACEHOLDER from '~/images/banner_placeholder.webp';
import { SmallSpinner } from '../spinner';

const readex_pro = Readex_Pro({ subsets: ['latin'] });

export const Header = ({ data, user, isLoading, isEditable = false }: HeaderProps) => {
  ...
  const initializeState = () => {
    if (data) {
      console.log('?????', data.user.bannerImage);
      setBannerImage(data.user.bannerImage);
      setProfileImage(data.user.avatarImage);
    }
  ...
  return (
    <div
      className={`flex flex-col justify-start h-auto mx-auto mt-4 xs:mt-10 mb-4 pb-10 relative bg-dark-green`}
      style={{
      ...
      <div className="flex flex-col 2xl:flex-row">
        <div className="flex flex-col">
          <div className="relative z-10 mx-auto mt-24 xs:mt-0 xs:ml-60">
            <h1 className={`${readex_pro.className} mt-5 lg:text-4xl text-3xl`}>{user?.dataState?.username}</h1>
          </div>
          ...
          <div className="mb-5 relative z-10 mt-2 px-5 lg:px-0 lg:ml-60">
            {isEditable ? (
              ...
              <div>
                <div className={`${readex_pro.className} text-grey font-normal mt-2 h-auto w-full max-h-60`}>{bio}</div>
              </div>
            )}
          </div>
          {updating && (
