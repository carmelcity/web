...
const ActionButton = (props: any) => {
  ...
  return (
    <a className={`btn ml-2 ${props.primary ? 'btn-primary' : ''} text-white`} href={props.link}>
...
const IconButton = (props: any) => {
  const Icon = props.icon;
  return (
    <a className={`btn btn-ghost rounded-full ml-2 text-white`} href={props.link}>
...
const ActionButtons = (props: any) => {
  if (props.carmel.auth.account) {
    return [
      <IconButton key={0} link="/" title={props.carmel.auth.account.user.username} primary icon={UserCircleIcon} />,
    ];
  }
  ...
  return (
    <header className="navbar bg-base-100 sticky top-0 z-50 w-full h-20 bg-black/40 backdrop-blur-xl flex justify-center items-center border-b border-primary-color">
      <div className="navbar-start">
        <Link className="btn btn-ghost rounded-full text-white" href="/">
      ...
      <div className="navbar-end">{ActionButtons(props)}</div>
    </header>
  );
};
