...
export function MobileSidebar({ section, sections, sideBar, setSideBar, onMobileItemSelect }: any) {
  ...
  const NavItem = (item: any) => {
    ...
    return (
      <div tabIndex={0} className="text-left p-4 flex flex-col items-start">
        <span className="text-xl opacity-50">{item.title}</span>
        {item.sections.map((subitem: any, i: number) => (
          <button key={i} className="ml-4 text-lg text-white mt-2" onClick={() => onMobileItemSelect(subitem)}>
            {subitem.title}
          </button>
        ))}
      </div>
