...
export default ({}: any) => {
  ...
  const SubmenuItem = (item: any) => {
    return (
      <button
        onClick={() => changeSubitem(item)}
        className={`${
          item.id === section.id
            ? 'text-primary-color font-black border-b border-primary-color bg-primary-color bg-opacity-20'
            : 'hover:bg-opacity-20 hover:bg-black'
        } p-2 cursor-pointer mt-2 transition transform hover:-translate-y-1 motion-reduce:transition-none motion-reduce:hover:transform-none ml-8 text-md text-left`}>
        {item.title}
      </button>
    );
  };
  ...
  const MenuItem = (item: any) => {
    return (
      <div className="flex flex-col">
        <div className="ml-4 mt-4 text-md text-left opacity-50">{item.title}</div>
  ...
  const Document = () => {
    ...
    return (
      <div className="content break-words text-left">
        ...
        <ReactMarkdown children={doc.data} components={CodeBlock} remarkPlugins={[remarkBreaks]} />
      </div>
    );
  };

  return (
    <div className="flex lg:flex-row h-auto flex-col w-full">
