...
export const Avatar = ({ username, appLogo, appName, alt, profileImage, communityName }: any) => {
  return (
    <div className="flex">
      ...
      <div className={`${readexPro.className} block items-center font-normal justify-center ml-3 max-w-1/2`}>
        ...
        <div className={`flex w-auto`}>
          <span className={`${readexPro.className} text-xs text-gray-400 font-thin flex`}>published in</span>
          <Link
            href={`/${communityName}`}
            className={`${readexPro.className} text-xs text-cyan text-s font-thin ml-1.5 hover:underline`}>
            {communityName}
          </Link>
        </div>
      </div>
