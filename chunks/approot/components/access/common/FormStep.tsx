...
export default ({ text, user, fields, setFields, setFinished, handlers }: any) => {
  ...
  const handleContinue = async () => {
    ...
    switch (currentIndex) {
      ...
      case 1:
        ...
        break;
      case 2:
        result = await handlers[currentIndex].onLoad({
          query,
          value: { ...fields, [text.form.steps[currentIndex].key]: value || true },
        });
        if (!result) {
          setError(text.form.steps[currentIndex].error);
        } else {
  ...
  const Error = () => {
    ...
    return (
      <div className="p-4 mb-4 text-sm bg-gray-800 text-red-400" role="alert">
        {error}
      </div>
    );
  };
  ...
  return (
    <div className="xl:w-full mx-5 items-center lg:mx-32 xl:mx-0 h-auto bg-transparent p-10 rounded-md shadow-lg backdrop-blur-lg border border-[2px] border-dark-turquoise z-30 relative mt-10 xl:mt-0">
      ...
      {loading || (
        <button
          ...
          className={`${readexPro.className} bg-cyan ${
            enableAction ? 'bg-opacity-80 hover:bg-opacity-90' : 'bg-opacity-20'
          } text-black py-2 px-4 focus:ring focus:ring-cyan focus:ring-opacity-50 mt-10`}
