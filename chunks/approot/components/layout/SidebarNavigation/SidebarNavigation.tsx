...
import { SidebarNavigationPlaceholder } from '../../placeholders/SidebarNavigation';
...
export const SidebarNavigation = ({ data, isLoading, user }: SidebarNavigationProps) => {
  ...
  return (
    <div className="flex z-10 grow sticky relative -top-10 flex-col gap-y-5 overflow-y-auto bg-gradient-to-br from-dark-green to-dark-green min-w-[300px] px-6 w-full mr-auto md:h-screen border-r border-cyan/10">
              ...
              {navbarElements.map((item, index) => (
                <li key={item.name}>
                  <Link
                    ...
                    <Image src={item.current ? item.iconActive : item.icon} alt="card" className="object-fit" />
                    {item.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="mr-auto ml-1 bg-transparent flex justify-center items-center">
          <div className="flex items-center">
            ...
            <div className="flex items-center justify-center ml-3">
              <span className={`${readexPro.className} text-cyan text-s font-extralight`}>{user.account.username}</span>
            </div>
          </div>
        </div>
        <div
          className="w-9 h-full ml-auto bg-dark-green-secondary flex justify-center items-center cursor-pointer"
