...
const Item = (item: any) => {
  ...
  return (
    <Link
      href={item.path}
      className={`${item.active ? 'text-cyan bg-dark-green-secondary ' : 'text-gray-400'} ${
        readexPro.className
...
export default () => {
  ...
  return (
    <nav className="flex flex-1 flex-col mt-2 w-full">
      <ul role="list" className="flex flex-1 flex-col gap-y-7">
        <li>
          <ul role="list" className="-mx-2 space-y-1">
            {SideMenuItems.map((item: any, i: number) => (
              <Item key={i} {...item} active={pathname === item.path} />
            ))}
          </ul>
        </li>
      </ul>
